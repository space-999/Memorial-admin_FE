...
export const FlowerMessages: React.FC = () => {
  ...
  const fetchMessages = async () => {
    try {
      setLoading(true);
      const data = await apiClient.getFlowerMessages() as FlowerMessage[];
  ...
  const handleSave = async (messageData: Partial<FlowerMessage>) => {
    try {
      if (editingMessage) {
        await apiClient.updateFlowerMessage(editingMessage.messageId, messageData);
        toast({
          title: '수정 완료',
            ...
            <TableBody>
              {messages.map((message) => (
                <TableRow key={message.messageId}>
                  ...
                  <TableCell>
                    <Badge variant={message.isVisible ? 'default' : 'secondary'}>
                      {message.isVisible ? '표시' : '숨김'}
                    </Badge>
                  </TableCell>
                  <TableCell>
